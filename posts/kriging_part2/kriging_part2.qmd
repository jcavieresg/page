...
range   <- 0.25      # Matérn length-scale ℓ
nu      <- 1         # Matérn smoothness ν
sigma_u <- 1.0       # process SD σ
sigma_e <- 0.15      # nugget (measurement noise) SD σ_e
mu_true <- 0.0       # constant mean μ
...
#------------------------------------------------------
# Observation locations: N_s uniform points in [0,1]²
#------------------------------------------------------
obs_locs <- matrix(runif(N_s * 2), ncol = 2, dimnames = list(NULL, c("s1", "s2")))

#------------------------------------------------------
# Simulate the latent field U(s) ~ GP(0, sigma2 C) via Cholesky:
#------------------------------------------------------
Sigma_U <- matern_cov(obs_locs, nu = nu, range = range, sigma2 = sigma_u^2)
diag(Sigma_U) <- diag(Sigma_U) + 1e-9     # small jitter for numerical stability
L_U    <- t(chol(Sigma_U))
...
# Cross-covariance  k_* = σ² c_*  (N_pred × N_obs matrix)
#------------------------------------------------------
...
# True latent field at observation locations
...
In this post we have developed Kriging prediction unsder Bayesian inference. Here, the GRF assumption allow us making spatial interpolation into a conditional Gaussian inference: given observations $\mathbf{y}$, the posterior at any unobserved location $\mathbf{s}_*$ is a normal distribution whose mean $\hat{y}_*$ and variance $\sigma^2_*$ are available in closed form through simple linear algebra.
